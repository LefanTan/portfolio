...
export const Projects = memo((props: ProjectProp) => {
    ...
    return (
        <motion.div variants={parentVariant} initial='hidden' animate='visible' className={`${props.className} flex flex-col items-center justify-start w-1/3 h-full m-5`}>
            <div className={`w-fit ${props.taller ? 'h-1/2' : 'h-fit'} mb-5 mt-5 flex justify-center items-center p-2 relative`}>
                <video muted autoPlay loop src={getFile(props.imgFileName, "video")} className="z-10 h-full" />
                <div className="absolute bg-antique-white w-full h-full z-0" />
            </div>
            <div className="flex flex-col h-fit pt-5">
                ...
                <motion.p variants={opacityChildrenVariant} className="whitespace-pre-line font-main text-off-white font-light mt-3">{props.description}</motion.p>
                <div className="w-full flex justify-start items-center whitespace-pre mt-1">
                    <motion.p variants={opacityChildrenVariant} className="font-main italic text-antique-white mt-1 whitespace-pre">{props.languages.join("    ")}</motion.p>
                    <a href={props.github} target="_blank" rel="noopener noreferrer" className="w-7 ml-auto transition duration-100 transform hover:scale-110 active:scale-100">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" >
                            <motion.path variants={pathChildrenVariant} className="stroke-antique-white stroke-1 fill-transparent "
                    ...
                    {props.website && <a href={props.website} target="_blank" rel="noopener noreferrer" className="w-6 ml-1 transition duration-100 transform hover:scale-110 active:scale-100">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 420 420" className="fill-transparent">
                            <motion.path variants={pathChildrenVariant} strokeLinecap='round' strokeLinejoin='round'
